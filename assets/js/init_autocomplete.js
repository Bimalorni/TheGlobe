const initialize_autocomplete = () => {
  ...
  var searchedLongitude, searchedLatitude;
   ...
   $("#view-office-search-input").autocomplete({
        minLength: 0,
...
const init_air_travel_autocomplete = (data) => {
  ...
  $("#view-route-search-input").autocomplete({
    minLength: 0,
    ...
    select: function( event, ui ) {
      ...
      if(isNaN(lng_src) || isNaN(lng_dst) || isNaN(lat_src) || isNaN(lat_dst)) return;
...
const init_fleet_autocomplete = (data) => {
  ...
  $("#view-fleet-search-input").autocomplete({
    minLength: 0,
    source: autocompleteData,
    position: { 
      my: "left bottom",
